...
import api.payLoad.User;import org.apache.logging.log4j.LogManager;
...
public class UserTests {
	...
	{logger.info("**************creating  array of user ****************************");
		...
		Response response=UserEndPoints.createUser2(userpayload);
		response.then()
		.assertThat().
		statusCode(200).
		log().body().extract().response();
	...
	Response response=UserEndPoints.getUser2();
	response.then().log().all();
